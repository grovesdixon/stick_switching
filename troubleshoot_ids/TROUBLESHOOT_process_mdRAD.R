...
bed = read_tsv('mdRAD/data/geneBoundaries.bed.tsv') %>% 
  dplyr::select(-X101)
counts = bed %>% 
...
k=lengths/1e3
geneFpkm=sweep(counts, 1, k, `/`) %>% 
  sweep(2, m, `/`)

#get row varainces and select top
df = as.matrix(geneFpkm)
rv <- rowVars(df)
ntop=round(nrow(df)/10, 0)
...
pca_df = d %>% 
  ...
  mutate(pos1 = PC1 > 0,
         ...
         clusterN1 = !pos1 & !pos2,
         clusterN4 = pos1 & !pos2,
         cluster = if_else(clusterL1,
                           'L1clust',
                           'na'),
         cluster = if_else(clusterN1,
                           'N1clust',
                           cluster),
         cluster = if_else(clusterN4,
                           'N4clust',
                           cluster))
...
#check that the name mismatched are in here
pca_df %>% 
  filter(cluster=='N1clust')

#check that the name mismatched are in here
pca_df %>% 
  filter(cluster=='N4clust')
