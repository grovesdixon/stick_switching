...
if __name__ == '__main__':
  ...
  By default, the program only uses the R1 read for deduplication, to use the R2 read as well,
  change -preDedup to True
  '''
  ...
  parser.add_argument('-peDedup', required = False, default = False, dest = 'paired_end_dedup', help = 'Boolean for whether to use both R1 and R2 reads for deduplication. This may identify more false positive duplicates, but is more conservative')
  ...
  if peDedup:
    print('Using both pe reads for deduplication...')
    useDedupList = combinedDupList
  else:
    print('Using only R1 reads for deduplication...')
    useDedupList = r1DuplicateList
  finalCheckList = []
  for i in range(len(combinedCheckList)):
    finalCheckList.append( (combinedCheckList[i] and useDedupList[i])) #both need to be True to keep read
